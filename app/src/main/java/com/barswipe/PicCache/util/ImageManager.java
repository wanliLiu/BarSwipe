...
//                                            mMemoryCache.put(url + imageRef.width + imageRef.height, bitmap);
    ...
    public String urlToFilePath(String url) {
        ...
        if (!url.startsWith("http")) {
            ...
            //否则就是缓存目录的路径
            return mDiskCache.createFilePath(url);
        }
     ...
     * @return
     */
    public File getPicturePath(String path) throws FileNotFoundException {
        File file = null;

        if (!path.startsWith("http")) {//是本地图片
        } else {//网络图片
            //先看本地缓存有没有
            path = mDiskCache.createFilePath(path);
        }
        file = new File(path);
        if (file.exists()) {
            return file;
