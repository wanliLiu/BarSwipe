...
//            return mDecodedSamples.asReadOnlyBuffer();
            return mDecodedBytes.asShortBuffer();
        } else {
            return null;
        }
    }
    ...
    private void ReadFile(File inputFile)
        ...
        for (i = 0; i < numTracks; i++) {
            ...
            if (format.getString(MediaFormat.KEY_MIME).startsWith("audio/")) {
                extractor.selectTrack(i);//选中音频轨道
                break;
            }
        }
        if (i == numTracks) {
        ...
        while (true) {
            ...
            int inputBufferIndex = codec.dequeueInputBuffer(100);//获取可用的inputBuffer -1代表一直等待，0表示不等待 建议-1,避免丢帧
            if (!done_reading && inputBufferIndex >= 0) {
                sample_size = extractor.readSampleData(inputBuffers[inputBufferIndex], 0);////MediaExtractor读取数据到inputBuffer中
                if (firstSampleData
            ...
            if (outputBufferIndex >= 0 && info.size > 0) {
                ...
                outputBuffers[outputBufferIndex].get(decodedSamples, 0, info.size);//拿到用于存放PCM数据的Buffer
        ...
        while (true) {
            ...
            if (!done_reading && inputBufferIndex >= 0) {
                if (num_samples_left <= 0) {
                    ...
                    inputBuffers[inputBufferIndex].clear();//清空之前传入inputBuffer内的数据
                    ...
                    inputBuffers[inputBufferIndex].put(buffer);//PCM数据填充给inputBuffer
                    presentation_time = (long) (((num_frames++) * frame_size * 1e6) / mSampleRate);
                    codec.queueInputBuffer(inputBufferIndex, 0, buffer.length, presentation_time, 0);//通知编码器 编码
                }
            }

            // Get the encoded samples from the encoder.获取编码后的数据
            int outputBufferIndex = codec.dequeueOutputBuffer(info, 100);
            if (outputBufferIndex >= 0 && info.size > 0 && info.presentationTimeUs >= 0) {
                ...
                outputBuffers[outputBufferIndex].get(encodedSamples, 0, info.size);//把编码后的数据放到encodedSamples数组中
                outputBuffers[outputBufferIndex].clear();//清除
                codec.releaseOutputBuffer(outputBufferIndex, false);//数据拿出来后就释放所占用的缓存
                ...
                encodedBytes.put(encodedSamples, 0, info.size);//把界面的数据从encodedSamples放到encodedBytes中
            } else if (outputBufferIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                outputBuffers = codec.getOutputBuffers();
            } else if (outputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                // Subsequent data will conform to new format.
